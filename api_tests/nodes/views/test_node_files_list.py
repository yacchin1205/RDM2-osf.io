...
class TestNodeFilesList(ApiTestCase):
    ...
    def test_vol_node_files_list(self):
        ...
        assert (url.query) == ('meta=True&view_only={}'.format(str(vol.key)))
    ...
    def test_handles_request_to_provider_not_configured_on_project(self):
        ...
        assert (res.status_code) == (404)
        assert (res.json['errors'][0]['detail']) == ('The {} provider is not configured for this project.'.format(provider))
...
class TestNodeStorageProviderDetail(ApiTestCase):
    ...
    def test_can_view_if_contributor(self):
        res = self.app.get(self.private_url, auth=self.user.auth)
        assert (res.status_code) == (200)
        assert (res.json['data']['id']) == ('{}:osfstorage'.format(self.private_project._id))

    def test_can_view_if_public(self):
        res = self.app.get(self.public_url)
        assert (res.status_code) == (200)
        assert (res.json['data']['id']) == ('{}:osfstorage'.format(self.public_project._id))
