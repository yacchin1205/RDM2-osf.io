...
class TestAuthViews(DataverseAddonTestCase):
    ...
    def test_deauthorize(self):
        url = api_url_for('weko_deauthorize_node',
                          pid=self.project._primary_key)
        self.app.delete(url, auth=self.user.auth)

        self.node_settings.reload()
        assert_false(self.node_settings.weko_alias)
        ...
        assert_equal(last_log.action, 'weko_node_deauthorized')
    ...
    def test_user_config_get(self):
        url = api_url_for('weko_user_config_get')
...
class TestConfigViews(DataverseAddonTestCase, views.OAuthAddonConfigViewsTestCaseMixin):
    ...
    def setUp(self):
        ...
        self.mock_ser_api = mock.patch('website.addons.weko.serializer.client.connect_from_settings')
    ...
    @mock.patch('website.addons.weko.views.client.connect_from_settings')
    def test_folder_list(self, mock_connection):
        #test_get_datasets
        mock_connection.return_value = self.connection

        url = api_url_for('weko_get_datasets', pid=self.project._primary_key)
        params = {'alias': 'ALIAS1'}
    ...
    def test_set_config(self, mock_connection):
        ...
        assert_equal(res.json['dataverse'], self.connection.get_weko('ALIAS3').title)
        assert_equal(res.json['dataset'], 
            self.connection.get_weko('ALIAS3').get_dataset_by_doi('doi:12.3456/DVN/00003').title)
    ...
    def test_set_config_no_dataset(self, mock_connection):
        ...
        url = api_url_for('weko_set_config',
                          pid=self.project._primary_key)
        params = {
            'dataverse': {'alias': 'ALIAS3'},
        ...
        assert_equal(self.node_settings.weko_alias, 'ALIAS2')
...
class TestHgridViews(DataverseAddonTestCase):
    ...
    @mock.patch('website.addons.weko.views.client.connect_from_settings')
    @mock.patch('website.addons.weko.views.client.get_files')
    def test_weko_root_published(self, mock_files, mock_connection):
        ...
        self.node_settings.weko_alias = alias
        self.node_settings.dataset_doi = doi
        self.node_settings.save()

        url = api_url_for('weko_root_folder',
                          pid=self.project._primary_key)
    ...
    @mock.patch('website.addons.weko.views.client.connect_from_settings')
    @mock.patch('website.addons.weko.views.client.get_files')
    def test_weko_root_not_published(self, mock_files, mock_connection):
        mock_connection.return_value = create_mock_connection()
    ...
    @mock.patch('website.addons.weko.views.client.connect_from_settings')
    @mock.patch('website.addons.weko.views.client.get_files')
    def test_weko_root_no_connection(self, mock_files, mock_connection):
        mock_connection.return_value = create_mock_connection()
        mock_files.return_value = ['mock_file']
    ...
    def test_weko_root_incomplete(self):
...
class TestCrudViews(DataverseAddonTestCase):
    ...
    @mock.patch('website.addons.weko.views.client.connect_from_settings_or_401')
    @mock.patch('website.addons.weko.views.client.publish_dataset')
    @mock.patch('website.addons.weko.views.client.publish_weko')
    def test_weko_publish_dataset(self, mock_publish_dv, mock_publish_ds, mock_connection):
        mock_connection.return_value = create_mock_connection()

        url = api_url_for('weko_publish_dataset',
                          pid=self.project._primary_key)
    ...
    @mock.patch('website.addons.weko.views.client.publish_dataset')
    @mock.patch('website.addons.weko.views.client.publish_weko')
    def test_weko_publish_both(self, mock_publish_dv, mock_publish_ds, mock_connection):
        mock_connection.return_value = create_mock_connection()
...
class TestDataverseRestrictions(DataverseAddonTestCase):
    ...
    def test_restricted_set_dataset_not_owner(self, mock_connection):
        ...
        url = api_url_for('weko_set_config', pid=self.project._primary_key)
        params = {
