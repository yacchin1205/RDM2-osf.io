...
from website.addons.weko.tests.utils import DataverseAddonTestCase
from website.addons.weko.tests.utils import create_external_account
from website.addons.weko.client import (
    _connect, get_files, publish_dataset, get_datasets, get_dataset,
    get_wekos, get_weko, connect_from_settings, connect_or_error,
...
class TestClient(DataverseAddonTestCase):
    ...
    def setUp(self):
        ...
        self.mock_weko = mock.create_autospec(Dataverse)
        ...
        self.mock_dataset.dataverse = self.mock_weko
        self.mock_weko.connection = self.mock_connection

    @mock.patch('website.addons.weko.client.Connection')
    ...
    @mock.patch('website.addons.weko.client._connect')
    ...
    def test_publish_dataset_unpublished_weko(self):
        type(self.mock_weko).is_published = mock.PropertyMock(return_value=False)
        with assert_raises(HTTPError) as e:
            publish_dataset(self.mock_dataset)
    ...
    def test_get_datasets(self):
        ...
        self.mock_weko.get_datasets.return_value = [
            mock_dataset1, mock_dataset2, mock_dataset3
        ]

        datasets = get_datasets(self.mock_weko)
        assert_is(self.mock_weko.get_datasets.assert_called_once_with(timeout=settings.REQUEST_TIMEOUT), None)
        assert_in(mock_dataset1, datasets)
        assert_in(mock_dataset2, datasets)
        assert_in(mock_dataset3, datasets)

    def test_get_datasets_no_weko(self):
        datasets = get_datasets(None)
        assert_equal(datasets, [])

    def test_get_dataset(self):
        self.mock_dataset.get_state.return_value = 'DRAFT'
        self.mock_weko.get_dataset_by_doi.return_value = self.mock_dataset

        s = get_dataset(self.mock_weko, 'My hdl')
        assert_is(self.mock_weko.get_dataset_by_doi.assert_called_once_with('My hdl', timeout=settings.REQUEST_TIMEOUT), None)
    ...
    def test_get_wekos(self):
        ...
        self.mock_connection.get_wekos.return_value = [
            published_dv, unpublished_dv
        ]

        dvs = get_wekos(self.mock_connection)
        self.mock_connection.get_wekos.assert_called_once_with()

        assert_in(published_dv, dvs)
        assert_in(unpublished_dv, dvs)
        assert_equal(len(dvs), 2)

    def test_get_weko(self):
        type(self.mock_weko).is_published = mock.PropertyMock(return_value=True)
        self.mock_connection.get_weko.return_value = self.mock_weko

        d = get_weko(self.mock_connection, 'ALIAS')
        self.mock_connection.get_weko.assert_called_once_with('ALIAS')

        assert_equal(d, self.mock_weko)

    def test_get_unpublished_weko(self):
        type(self.mock_weko).is_published = mock.PropertyMock(return_value=False)
        self.mock_connection.get_weko.return_value = self.mock_weko
