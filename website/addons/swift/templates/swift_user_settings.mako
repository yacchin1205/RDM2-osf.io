<!-- Authorization -->
<div id='swiftAddonScope' class='addon-settings addon-generic scripted'
     data-addon-short-name="${ addon_short_name }"
     data-addon-name="${ addon_full_name }">

    <%include file="swift_credentials_modal.mako"/>

    <h4 class="addon-title">
        <img class="addon-icon" src=${addon_icon_url}>
        <span data-bind="text: properName"></span>
        <small>
            <a href="#swiftInputCredentials" data-toggle="modal" class="pull-right text-primary">Connect Account</a>
        </small>
    </h4>
