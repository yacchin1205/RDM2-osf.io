...
from website.addons.azureblobstorage.tests.utils import S3AddonTestCase
from website.addons.azureblobstorage.utils import validate_bucket_name, validate_bucket_location
from website.util import api_url_for


class TestS3Views(S3AddonTestCase, testing.views.OAuthAddonConfigViewsTestCaseMixin):
    def setUp(self):
        self.mock_can_list = mock.patch('website.addons.azureblobstorage.views.utils.can_list')
        self.mock_can_list.return_value = True
        self.mock_can_list.start()
        self.mock_uid = mock.patch('website.addons.azureblobstorage.views.utils.get_user_info')
        self.mock_uid.return_value = {'id': '1234567890', 'display_name': 'azureblobstorage.user'}
        self.mock_uid.start()
        self.mock_exists = mock.patch('website.addons.azureblobstorage.views.utils.container_exists')
    ...
    @mock.patch('website.addons.azureblobstorage.views.utils.can_list', return_value=False)
    ...
    @mock.patch('website.addons.azureblobstorage.model.get_bucket_names')
    def test_folder_list(self, mock_names):
        mock_names.return_value = ['bucket1', 'bucket2']
        super(TestS3Views, self).test_folder_list()

    @mock.patch('website.addons.azureblobstorage.model.container_exists')
...
class TestCreateBucket(S3AddonTestCase):
    ...
    @mock.patch('website.addons.azureblobstorage.views.utils.create_container')
    @mock.patch('website.addons.azureblobstorage.views.utils.get_bucket_names')
    def test_create_bucket_pass(self, mock_names, mock_make):
        ...
        url = self.project.api_url_for('azureblobstorage_create_bucket')
