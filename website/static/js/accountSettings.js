    ...
    update: function (profile, email) {
        ...
        var request = $osf.putJSON(
            ...
            if (xhr.status === 400) {
                ...
                'again or contact <a href="mailto: nii-rdmp@meatmail.jp">nii-rdmp@meatmail.jp</a> ' +
...
var ExternalIdentityViewModel = oop.defclass({
    ...
    _removeIdentity: function(identity) {
        ...
        request.fail(function(xhr, status, error) {
            $osf.growl('Error',
                'Revocation request failed. Please contact <a href="mailto: nii-rdmp@meatmail.jp">nii-rdmp@meatmail.jp</a> if the problem persists.',
                'danger'
            );
    ...
    _requestExport: function() {
        ...
        request.fail(function(xhr, status, error) {
            if (xhr.responseJSON.error_type === 'throttle_error') {
                ...
                $osf.growl('Error',
                    'Export request failed. Please contact <a href="mailto: nii-rdmp@meatmail.jp">nii-rdmp@meatmail.jp</a> if the problem persists.',
                    'danger'
                );
            }
