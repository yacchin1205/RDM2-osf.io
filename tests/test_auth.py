...
class TestAuthUtils(OsfTestCase):
    ...
    def test_successful_external_login_cas_redirect(self, mock_service_validate, mock_get_user_from_cas_resp):
        ...
        assert 'username={}'.format(username_quoted) in resp.location
        assert 'verification_key={}'.format(user.verification_key) in resp.location
    ...
    def test_password_change_sends_email(self, mock_mail):
        ...
        assert empty == ()
        assert kwargs == {'user': user, 'mimetype': 'html', 'mail': mails.PASSWORD_RESET, 'to_addr': user.username, 'can_change_preferences': False, 'osf_contact_email': settings.OSF_CONTACT_EMAIL}
    ...
    def test_sign_up_twice_sends_two_confirmation_emails_only(self, mock_mail):
        ...
        assert args == ('caesar@romanempire.com', mails.INITIAL_CONFIRM_EMAIL, 'html')
