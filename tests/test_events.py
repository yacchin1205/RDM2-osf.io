...
class TestFileUpdated(OsfTestCase):
    ...
    def test_info_formed_correct(self):
        assert '{}_file_updated'.format(wb_path) == self.event.event_type
...
class TestFileRemoved(NotificationTestCase):
    ...
    def test_info_formed_correct_folder(self):
        ...
        assert u'removed folder "<b>{}/</b>".'.format(materialized.lstrip('/')) == self.event.html_message
        assert u'removed folder "{}/".'.format(materialized.lstrip('/')) == self.event.text_message
...
class TestFileCopied(NotificationTestCase):
    ...
    def test_info_correct(self):
        # Move Event: Ensures data is correctly formatted
        assert '{}_file_updated'.format(wb_path) == self.event.event_type
        assert 'copied file "<b>One/Paper13.txt</b>" from OSF Storage in Consolidate to "<b>Two/Paper13.txt</b>" in OSF Storage in Consolidate.' == self.event.html_message
        assert 'copied file "One/Paper13.txt" from OSF Storage in Consolidate to "Two/Paper13.txt" in OSF Storage in Consolidate.' == self.event.text_message
