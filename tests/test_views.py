...
class TestViewingProjectWithPrivateLink(OsfTestCase):
    ...
    def test_logged_in_has_key(self):
        res = self.app.get(
            self.project_url, query_string={'view_only': self.link.key}, auth=self.user.auth)
        assert res.status_code == 200
...
class TestProjectViews(OsfTestCase):
    ...
    def test_node_setting_with_multiple_matched_institution_email_domains(self):
        ...
        user.reload()
        assert inst1 in user.affiliated_institutions.all()
        assert inst2 in user.affiliated_institutions.all()
    ...
    def test_project_remove_other_not_admin(self):
        ...
        assert res.json['message_long'] == 'You do not have permission to perform this action. If this should not have occurred and the issue persists, ' + language.SUPPORT_LINK
    ...
    def test_cant_remove_component_if_not_admin(self):
        ...
        url = node.api_url
        res = self.app.delete(url, json={}, auth=non_admin.auth)
    ...
    def test_retraction_view(self):
        ...
        for route in ['files', 'wiki/home', 'settings', 'withdraw', 'register', 'register/fakeid']:
            res = self.app.get('{}{}/'.format(url, route), auth=self.auth)
            assert res.status_code == 302, route
            res = self.app.get(res.location, auth=self.auth, follow_redirects=True)
...
class TestUserProfile(OsfTestCase):
    ...
    def test_unserialize_social(self):
        ...
        self.user.reload()
        assert self.user.social['profileWebsites'] == payload['profileWebsites']
        assert self.user.social['twitter'] == payload['twitter']
        assert self.user.social['github'] == payload['github']
    ...
    @mock.patch('framework.auth.views.mails.send_mail')
    @mock.patch('website.profile.views.mailchimp_utils.subscribe_mailchimp')
    @mock.patch('website.profile.views.mailchimp_utils.unsubscribe_mailchimp_async')
    def test_update_user_mailing_lists(self, mock_unsubscribe_mailchimp_async, mock_subscribe_mailchimp, send_mail):
        email = fake_email()
        ...
        self.app.put(url, json=payload, auth=self.user.auth)
        mock_unsubscribe_mailchimp_async.assert_called_with(
            list_name,
            self.user._id,
            username=self.user.username,
        )
        mock_subscribe_mailchimp.assert_called_with(
            list_name,
            self.user._id,
        )
    ...
    @mock.patch('website.profile.views.mailchimp_utils.subscribe_mailchimp')
    @mock.patch('website.profile.views.mailchimp_utils.unsubscribe_mailchimp_async')
    def test_unsubscribe_mailchimp_not_called_if_user_not_subscribed(self, mock_unsubscribe_mailchimp_async, mock_subscribe_mailchimp, send_mail):
        email = fake_email()
        self.user.emails.create(address=email)
        list_name = 'foo'
        ...
        assert mock_unsubscribe_mailchimp_async.call_count == 0
        assert mock_subscribe_mailchimp.call_count == 0
    ...
    def test_user_with_quickfiles(self):
        ...
        assert 'Quick files' in res.text
    ...
    def test_user_with_no_quickfiles(self):
        ...
        assert 'Quick files' not in res.text
    ...
    def test_append_idp_attr_common(self):
        ...
        assert data['idp_attr']['department_ja'] == self.user.ext.data['idp_attr']['organizational_unit_ja']
...
class TestAddingContributorViews(OsfTestCase):
    ...
    def test_email_sent_when_reg_user_is_added(self, send_mail):
        ...
        assert abs(contributor.contributor_added_email_records[project._id]['last_sent'] - int(time.time())) <= 1
...
class TestUserInviteViews(OsfTestCase):
    ...
    def test_invite_contributor_requires_fullname(self):
        res = self.app.post(
            self.invite_url,
            json={'email': 'brian@queen.com', 'fullname': ''},
            auth=self.user.auth,
        )
        assert res.status_code == http_status.HTTP_400_BAD_REQUEST
...
class TestClaimViews(OsfTestCase):
    ...
    def test_claim_user_post_with_registered_user_id(self, send_mail):
        ...
        assert res.json == {'status': 'success', 'email': reg_user.username, 'fullname': self.given_name}
    ...
    def test_claim_user_when_user_is_registered_with_orcid(self, mock_response_from_ticket):
        ...
        assert res1.headers.get('Location') == expected
        ...
        url_with_service_ticket = '{}?ticket={}'.format(url, service_ticket)
        ...
        osf_cookie = cookie_dict.get(settings.COOKIE_NAME)
        assert osf_cookie is not None

        g.current_session = None
        self.app.set_cookie(settings.COOKIE_NAME, osf_cookie, domain='localhost', origin_only=False)
        res = self.app.get(url)
        assert res.status_code == 302
    ...
    def test_posting_to_claim_form_removes_all_unclaimed_data(self, mock_update_search_nodes):
        ...
        self.user.add_unclaimed_record(p2, referrer=self.referrer,
                                       given_name=fake.name())
        self.user.save()
        assert len(self.user.unclaimed_records.keys()) > 1
        url = self.user.get_claim_url(self.project._primary_key)
    ...
    def test_claim_user_post_returns_fullname(self, send_mail):
        ...
        assert send_mail.called
        assert send_mail.call_args[0][0] == self.given_email
    ...
    def test_claim_user_post_if_email_is_different_from_given_email(self, send_mail):
        ...
        assert call_to_invited[1][0] == email
        call_to_referrer = send_mail.mock_calls[1]
        assert call_to_referrer[1][0] == self.referrer.username
    ...
    def test_cannot_claim_user_with_user_who_is_already_contributor(self):
        ...
        url = self.user.get_claim_url(self.project._primary_key)
        res = self.app.get(url, auth=contrib.auth, follow_redirects=True)
        # Response is a 400
        assert res.status_code == 400
...
class TestPointerViews(OsfTestCase):
    ...
    def test_pointer_list_read_contributor_cannot_remove_private_component_entry(self):
        ...
        pointer_nodes = html.fromstring(res.text).xpath('//li[@node_id]')
        has_controls = html.fromstring(res.text).xpath('//li[@node_id]/p[starts-with(normalize-space(text()), "Private Link")]//i[contains(@class, "remove-pointer")]')
    ...
    def test_forking_pointer_works(self):
        ...
        assert fork['title'] == 'Fork of {}'.format(linked_node.title)
...
class TestAuthViews(OsfTestCase):
    ...
    def test_register_ok(self, _):
        ...
        assert user.accepted_terms_of_service == None
    ...
    def test_register_sends_user_registered_signal(self, mock_send_confirm_email):
        ...
        assert mock_signals.signals_sent() == set([auth.signals.user_registered, auth.signals.unconfirmed_user_created])
    ...
    def test_resend_confirmation(self, send_mail):
        ...
        assert send_mail.called
        assert send_mail.call_count == 1
        assert send_mail.call_args[0][0] == email
        self.user.reload()
...
class TestAuthLoginAndRegisterLogic(OsfTestCase):
    ...
    def test_next_url_login_without_auth(self):
        ...
        assert data.get('status_code') == http_status.HTTP_302_FOUND
        assert data.get('next_url') == get_login_url(request.url)
    ...
    def test_institution_login_without_auth(self):
        ...
        assert data.get('status_code') == http_status.HTTP_302_FOUND
        assert data.get('next_url') == get_login_url(web_url_for('dashboard', _absolute=True), campaign='institution')
    ...
    def test_institution_login_next_url_without_auth(self):
        ...
        assert data.get('status_code') == http_status.HTTP_302_FOUND
        assert data.get('next_url') == get_login_url(self.next_url, campaign='institution')
    ...
    def test_campaign_login_without_auth(self):
        for campaign in get_campaigns():
            ...
            assert data.get('status_code') == http_status.HTTP_302_FOUND
            assert data.get('next_url') == web_url_for('auth_register', campaign=campaign, next=campaign_url_for(campaign))
    ...
    def test_campaign_register_without_auth(self):
        for campaign in get_campaigns():
            ...
            if is_native_login(campaign):
                ...
                assert data.get('next_url') == web_url_for('auth_login', next=campaign_url_for(campaign), _absolute=True)
...
class TestConfigureMailingListViews(OsfTestCase):
    ...
    @mock.patch('website.profile.views.mailchimp_utils.get_list_name_from_id')
    def test_sync_data_from_mailchimp_unsubscribes_user(self, mock_get_list_name_from_id):
        list_id = '12345'
        list_name = 'OSF General'
        mock_get_list_name_from_id.return_value = list_name
...
class TestFileViews(OsfTestCase):
    ...
    def test_grid_data_for_icon(self):
        ...
        data = res.json['data']
        assert len(data) == len(expected)
        assert data[0]['children'][0]['iconUrl'] == '/static/addons/osfstorage/comicon_custom_storage.png'
        assert data[0]['children'][0]['addonFullname'] == data[0]['children'][0]['nodeRegion']
...
class TestProjectCreation(OsfTestCase):
    ...
    def test_project_before_template_no_addons(self):
        ...
        assert 'Metadata' in res.json['prompts']
    ...
    def test_project_new_from_template_non_user(self):
        ...
        assert res.status_code == 302
        res2 = self.app.get(res.location)
    ...
    def test_create_component_strips_html_limit_project_number_error(self, _):
        ...
        url = web_url_for('project_new_node', pid=project._id)
        post_data = {'title': '<b>New <blink>Component</blink> Title</b>', 'category': ''}
        res = self.app.post(url, data=post_data, auth=user.auth)
        assert res.status_code == 403
...
class TestStaticFileViews(OsfTestCase):
    ...
    def test_getting_started_page(self):
        ...
        assert res.status_code == 302
        assert res.location == 'https://openscience.zendesk.com/hc/en-us'
...
class TestUserConfirmSignal(OsfTestCase):
    ...
    def test_confirm_user_signal_called_when_user_claims_account(self):
        ...
        assert mock_signals.signals_sent() == set([auth.signals.user_confirmed])
...
class TestResolveGuid(OsfTestCase):
    ...
    @mock.patch('website.views.requests.get')
    def test_preprint_provider_without_domain(self, mock_requests_get):
        ...
        res = self.app.get(url, buffered=True)
        assert res.status_code == 200
        mock_requests_get.assert_called()

    @mock.patch('website.views.requests.get')
    def test_preprint_provider_with_domain_without_redirect(self, mock_requests_get):
    ...
    def test_preprint_provider_with_domain_with_redirect(self):
        ...
        assert res.headers['location'] == '{}{}/'.format(domain, preprint._id)

        assert res.request.path == '/{}/'.format(preprint._id)

    @mock.patch('website.views.requests.get')
    def test_preprint_provider_with_osf_domain(self, mock_requests_get):
...
class TestConfirmationViewBlockBingPreview(OsfTestCase):
    ...
    def test_claim_user_form_cancel_request(self):
        ...
        claim_url = user.get_claim_url(project._primary_key)
        claim_url = '{}&cancel=true'.format(claim_url)
        res = self.app.get(
            claim_url,
        )
        assert res.status_code == 302
...
class TestTimestampView(OsfTestCase):
    ...
    def test_get_init_timestamp_error_data_list(self, mock_collection, mock_task):
        ...
        assert res.status_code == 200
        assert not TimestampTask.objects.filter(node=self.project).exists()
    ...
    def test_verify_timestamp_token(self, mock_get, mock_shutil, mock_aborted, mock_getfulllist, mock_checkfilets):
        ...
        assert mock_getfulllist.call_count == 1
        assert mock_checkfilets.call_count == 4
    ...
    def test_cancel_success(self, mock_task):
        ...
        cancel_res = self.app.post(
            url_cancel, json={}, auth=self.user.auth)

        assert cancel_res.status_code == 200
        assert cancel_res.json['success']
        assert mock_task.return_value.revoke.call_count == 1
        assert mock_task.return_value.abort.call_count == 1
        assert not TimestampTask.objects.filter(node=self.project).exists()
    ...
    def test_cancel_fail(self, mock_task):
        ...
        assert cancel_res.status_code == 200
        assert not cancel_res.json['success']
        assert mock_task.return_value.revoke.call_count == 0
        assert mock_task.return_value.abort.call_count == 0
        assert not TimestampTask.objects.filter(node=self.project).exists()
    ...
    def test_get_task_progress_ready(self, mock_task):
        ...
        status_res = self.app.post(
            url_progress, json={}, auth=self.user.auth)

        assert status_res.status_code == 200
        assert status_res.json['ready']
        assert not TimestampTask.objects.filter(node=self.project).exists()
    ...
    def test_get_task_progress_not_ready(self, mock_task):
        ...
        assert status_res.status_code == 200
        assert not status_res.json['ready']
        assert TimestampTask.objects.filter(node=self.project).exists()
...
class TestAddonFileViewTimestampFunc(OsfTestCase):
    ...
    def test_adding_timestamp(self, mock_collection):
        ...
        assert result['verify_result'] == 1

def test_get_storage_region_list_with_default():
    user = AuthUserFactory()
    institution = InstitutionFactory()
    user.affiliated_institutions.add(institution)
    assert len(website_view.get_storage_region_list(user)) == 1
...
def test_get_storage_region_list_with_own_institution():
    ...
    new_region.save()
    assert website_view.get_storage_region_list(user)[0]['name'] == new_region.name
