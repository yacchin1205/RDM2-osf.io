...
class TestUrlForHelpers(unittest.TestCase):
    ...
    def test_web_url_for_guid(self):
        with self.app.test_request_context():
            ...
            assert ('/nid321/') == (web_url_for('dummy_guid_project_view', pid='pid123', nid='nid321', _guid=True))
            assert ('/project/pid123/node/nid321/') == (web_url_for('dummy_guid_project_view', pid='pid123', nid='nid321', _guid=False))
            assert ('/project/pid123/node/nid321/') == (web_url_for('dummy_guid_project_view', pid='pid123', nid='nid321'))
            # check /profile/<pid>
    ...
    def test_web_url_for_guid_invalid_unicode(self):
        with self.app.test_request_context():
            ...
            assert ('/project/%C3%B8%CB%86%E2%88%86%E2%89%A4%C2%B5%CB%86/') == (web_url_for('dummy_guid_project_view', pid='øˆ∆≤µˆ', _guid=True))
            # check /project/<pid>/node/<nid>
            assert ('/ø∆≤µ©/') != (web_url_for('dummy_guid_project_view', pid='ø∆≤µ©', nid='©µ≤∆ø', _guid=True))
            assert ('/project/%C3%B8%CB%86%E2%88%86%E2%89%A4%C2%B5%CB%86/node/%C2%A9%C2%B5%E2%89%A4%E2%88%86%C3%B8/') == (web_url_for('dummy_guid_project_view', pid='øˆ∆≤µˆ', nid='©µ≤∆ø', _guid=True))
            # check /profile/<pid>
            assert ('/ø∆≤µ©/') != (web_url_for('dummy_guid_profile_view', pid='ø∆≤µ©', _guid=True))
            assert ('/profile/%C3%B8%CB%86%E2%88%86%E2%89%A4%C2%B5%CB%86/') == (web_url_for('dummy_guid_profile_view', pid='øˆ∆≤µˆ', _guid=True))
...
class TestFrameworkUtils(unittest.TestCase):
    ...
    def test_werkzeug_cases(self):
        ...
        assert ('i_contain_cool_umlauts.txt') == (secure_filename(u'i contain cool \xfcml\xe4uts.txt'))
...
class TestInspectInfo:
    def test_inspect_info(self):
        ...
        frame_info, ret = inspect.getframeinfo(current_frame), inspect_info(current_frame, stack_info)
        stack_first = stack_info[1]
        assert isinstance(ret, tuple)
        assert ret == (frame_info[0], frame_info[1], frame_info[2], stack_first[1], stack_first[2], stack_first[3])
