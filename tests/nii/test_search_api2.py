# from api_tests/search/views/test_views.py
...
class ApiSearchPrivateTestCase:
    ...
    def project_public(self, user_one):
        ...
        project_public.update_search()
        return project_public

    @pytest.fixture()
    def project_private(self, user_two):
        p = ProjectFactory(title='Coloring Book', creator=user_two)
        p.update_search()
        return p
    ...
    def component_public(self, user_two, project_public):
        ...
        component_public.update_search()
...
class TestSearchPrivate(ApiSearchPrivateTestCase):
    ...
    def test_search_results(
        ...
        # test_search_no_auth
        res = app.get(url_search, expect_errors=True)
...
class TestSearchPrivateComponents(ApiSearchPrivateTestCase):
    ...
    def test_search_components(
        ...
        res = app.get(url_component_search, expect_errors=True)
        assert res.status_code == 401

        # test_search_public_component_auth
        res = app.get(url_component_search, auth=user.auth)
        assert res.status_code == 200
        ...
        res = app.get(url_component_search, auth=user_two.auth)
        assert res.status_code == 200
        num_results = len(res.json['data'])
        total = res.json['links']['meta']['total']
        assert num_results == 2
...
class TestSearchPrivateFiles(ApiSearchPrivateTestCase):
    ...
    def test_search_files(
            self, app, url_file_search, user, user_one, user_two,
            file_public, file_component, file_private):

        # test_search_public_file_no_auth
        res = app.get(url_file_search, expect_errors=True)
        assert res.status_code == 401

        # test_search_public_file_auth
        res = app.get(url_file_search, auth=user.auth)
        assert res.status_code == 200
        num_results = len(res.json['data'])
        total = res.json['links']['meta']['total']
        assert num_results == 2
        ...
        res = app.get(url_file_search, auth=user_one.auth)
        assert res.status_code == 200
        num_results = len(res.json['data'])
        total = res.json['links']['meta']['total']
        ...
        assert file_public.name in res
        assert file_component.name in res
        assert file_private.name in res
        ...
        res = app.get(url_file_search, auth=user_one.auth)
        assert res.status_code == 200
        assert file_private.name in res  # can read

        res = app.get(url_file_search, auth=user_two.auth)
        assert res.status_code == 200
        assert file_private.name not in res  # can not read
...
class TestSearchPrivateProjects(ApiSearchPrivateTestCase):
    ...
    def test_search_projects(
        ...
        res = app.get(url_project_search, expect_errors=True)
        assert res.status_code == 401

        # test_search_public_project_auth
        res = app.get(url_project_search, auth=user.auth)
        assert res.status_code == 200
        num_results = len(res.json['data'])
        total = res.json['links']['meta']['total']
        assert num_results == 2
        ...
        res = app.get(url_project_search, auth=user_one.auth)
        assert res.status_code == 200
        num_results = len(res.json['data'])
        total = res.json['links']['meta']['total']
        assert num_results == 2
        ...
        res = app.get(url_project_search, auth=user_two.auth)
        assert res.status_code == 200
        assert project_private.title in res
...
@pytest.mark.django_db
class TestSearchPrivateRegistrations(ApiSearchPrivateTestCase):
    ...
    def registration_public(self, project_public, schema):
        with mock_archive(project_public, autocomplete=True, autoapprove=True, schema=schema) as registration_public:
            registration_public.update_search()
    ...
    def test_search_registrations(
        ...
        res = app.get(url_registration_search, expect_errors=True)
        assert res.status_code == 401

        # test_search_public_registration_auth
        res = app.get(url_registration_search, auth=user.auth)
        assert res.status_code == 200
        num_results = len(res.json['data'])
        total = res.json['links']['meta']['total']
        assert num_results == 2
        ...
        res = app.get(url_registration_search, auth=user_one.auth)
        assert res.status_code == 200
        num_results = len(res.json['data'])
        total = res.json['links']['meta']['total']
        assert num_results == 2
        ...
        res = app.get(url_registration_search, auth=user_two.auth)
        assert res.status_code == 200
        assert registration_private.title in res
...
class TestSearchPrivateUsers(ApiSearchPrivateTestCase):
    ...
    def test_search_user(self, app, url_user_search, user, user_one, user_two):
        ...
        res = app.get(url_user_search, expect_errors=True)
        assert res.status_code == 401

        # test_search_users_auth
        res = app.get(url_user_search, auth=user.auth)
        assert res.status_code == 200
        num_results = len(res.json['data'])
        total = res.json['links']['meta']['total']
        assert num_results == 3
...
class TestSearchPrivateInstitutions(ApiSearchPrivateTestCase):
    ...
    def test_search_institutions(
        ...
        res = app.get(url_institution_search, expect_errors=True)
        assert res.status_code == 401

        # test_search_institutions_auth
        res = app.get(url_institution_search, auth=user.auth)
        assert res.status_code == 200
        num_results = len(res.json['data'])
        total = res.json['links']['meta']['total']
        assert num_results == 1
...
class TestSearchPrivateCollections(ApiSearchPrivateTestCase):
    ...
    def node_two(self, user):
        n = NodeFactory(title='Sambolera', creator=user, is_public=True)
        n.update_search()
        return n
    ...
    def test_search_collections(
        ...
        res = app.get(url_collection_search, expect_errors=True)
        assert res.status_code == 401

        # test_search_collections_auth
        res = app.get(url_collection_search, auth=user.auth)
        assert res.status_code == 200
        total = res.json['links']['meta']['total']
        num_results = len(res.json['data'])
        assert total == 4
    ...
    def test_POST_search_collections(
        ...
        res = app.post_json_api(url_collection_search, payload, auth=user.auth)
        assert res.status_code == 200
