...
class CitationsUtilsTestCase(OsfTestCase):
    def test_datetime_to_csl(self):
        ...
        assert datetime_to_csl(now) == {'date-parts': [[now.year, now.month, now.day]]}
...
class CitationsNodeTestCase(OsfTestCase):
    ...
    def test_csl_single_author(self):
        ...
        assert self.node.csl == {'publisher': 'GakuNin RDM', 'author': [{'given': self.node.creator.given_name, 'family': self.node.creator.family_name}], 'URL': self.node.display_absolute_url, 'issued': datetime_to_csl(self.node.logs.latest().date), 'title': self.node.title, 'type': 'webpage', 'id': self.node._id}
    ...
    def test_csl_multiple_authors(self):
        ...
        assert self.node.csl == {'publisher': 'GakuNin RDM', 'author': [{'given': self.node.creator.given_name, 'family': self.node.creator.family_name}, {'given': user.given_name, 'family': user.family_name}], 'URL': self.node.display_absolute_url, 'issued': datetime_to_csl(self.node.logs.latest().date), 'title': self.node.title, 'type': 'webpage', 'id': self.node._id}
...
class CitationsViewsTestCase(OsfTestCase):
    ...
    def test_list_styles(self):
        ...
        assert len([style for style in response.json['styles'] if style.get('id') == 'bibtex']) == 1
    ...
    def test_list_styles_filter(self):
        ...
        assert len(response.json['styles']) == 1

        assert response.json['styles'][0]['id'] == 'bibtex'
