...
class TestPreprintProvider(OsfTestCase):
    ...
    def test_remove_provider(self):
        ...
        assert (self.preprint.provider) == (None)
...
class TestOnPreprintUpdatedTask(OsfTestCase):
    ...
    def test_unregistered_users_guids(self):
        ...
        assert {x.attrs['uri'] for x in node.get_related()} == {
            user.absolute_url,
            'mailto:{}'.format(user.username),
        }
...
class TestPreprintOsfStorage(OsfTestCase):
    ...
    def test_create_log(self):
        ...
        assert (self.preprint.logs.latest().action) == ('{0}_{1}'.format('osf_storage', action))
