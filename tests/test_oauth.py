...
class TestExternalProviderOAuth2GoogleDrive(OsfTestCase):
    ...
    def test_start_flow(self):
        ...
        with self.app.app.test_request_context('/oauth/connect/mock2/'):
            ...
            # check parameters
            assert params == {'state': [creds['state']], 'response_type': ['code'], 'client_id': [self.provider.client_id], 'redirect_uri': [web_url_for('oauth_callback', service_name=self.provider.short_name, _absolute=True)]}
