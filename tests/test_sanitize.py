...
class TestSanitize(unittest.TestCase):
    ...
    def test_unescape_html_additional_safe_characters(self):
        assert sanitize.unescape_entities('&lt;&gt; diamonds &amp; diamonds &lt;&gt;', safe={'&lt;': '<', '&gt;': '>'}) == '<> diamonds & diamonds <>'
        assert sanitize.unescape_entities(['&lt;&gt;&amp;'], safe={'&lt;': '<', '&gt;': '>'})[0] == '<>&'
        assert sanitize.unescape_entities(('&lt;&gt;&amp;',), safe={'&lt;': '<', '&gt;': '>'})[0] == '<>&'
        assert sanitize.unescape_entities({'key': '&lt;&gt;&amp;'}, safe={'&lt;': '<', '&gt;': '>'})['key'] == '<>&'
