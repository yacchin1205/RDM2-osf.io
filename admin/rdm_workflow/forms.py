...
class WorkflowEngineForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        boolean_fields = {'verify_ssl'}
        for name, field in self.fields.items():
            if name in boolean_fields:
                continue
            css_classes = field.widget.attrs.get('class', '')
            field.widget.attrs['class'] = f'{css_classes} form-control'.strip()
        self.fields['verify_ssl'].widget.attrs['class'] = 'form-check-input'
        self.fields['verify_ssl'].label = _('Verify SSL certificate')
        self.fields['keyset_url'].widget = forms.HiddenInput()
        self.fields['keyset_url'].required = False
...
class WorkflowEngineKeyForm(forms.ModelForm):
    ...
    public_key = forms.CharField(widget=forms.Textarea(attrs={'class': 'form-control', 'rows': 6}))
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['kid'].widget.attrs['class'] = 'form-control'
        self.fields['kid'].label = _('Key ID (kid)')
