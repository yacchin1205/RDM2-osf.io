...
class UserListView(RdmPermissionMixin, UserPassesTestMixin, View):
    ...
    def count_user_met_logic_condition(self, institution_id, logic_condition_query_string,
                                       include_osf_user_query_string, logic_condition_params, include_osf_user_params):
        """ Count user that met the logic condition use raw SQL with %s placeholder and params list to prevent SQL injection
        Args:
            + institution_id: Institution ID.
            + logic_condition_query_string: Logic condition query string with %s placeholders generated by generate_logic_condition_from_attribute.
            User-input values (attribute_value) are not directly set in the logic_condition_query_string (they are properly escaped).
            They are set via parameters passed to Django's connection.cursor,
            so they are properly escaped. (Reference: https://docs.djangoproject.com/en/5.2/topics/db/sql/#connections-and-cursors)
            Also, attribute_name and setting_type are whitelisted, so there are no SQL injection concerns.
            + include_osf_user_query: Logic condition query string with %s placeholders generated by generate_logic_condition_from_attribute.
            + logic_condition_params: List of parameters generated by generate_logic_condition_from_attribute.
            + include_osf_user_params: List of parameters generated by generate_logic_condition_from_attribute.
        Example:
            logic_condition_query_string = "data -> 'idp_attr' ->> 'eppn' = %s AND data -> 'idp_attr' ->> 'affiliation' = %s"
            logic_condition_params = ['user@example.com', 'member']
            include_osf_user_query_string = "u.username = %s"
            include_osf_user_params = ['user123']
        Returns:
            Count of user.
        """
        query = ''
    ...
    def get_user_list_met_logic_condition(self, institution_id, page, logic_condition_query_string,
        ...
        """
        Get user list that meets the logic condition using raw SQL with %s placeholders and parameter lists to prevent SQL injection.
        ...
        Example:
            logic_condition_query_string = "data -> 'idp_attr' ->> 'eppn' = %s AND data -> 'idp_attr' ->> 'affiliation' = %s"
            logic_condition_params = ['user@example.com', 'member']
            include_osf_user_query = "u.username = %s"
            include_osf_user_params = ['user123']
        Returns:
            List of user dicts.
        """
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            for attribute in attribute_list:
                if attribute.get('attribute_name') not in settings.ATTRIBUTE_NAME_LIST:
                    return JsonResponse({'error_message': 'attribute_name is invalid.'}, status=HTTPStatus.BAD_REQUEST)
                # Generate query string with %s placeholder and params list to prevent SQL injection
                if attribute.get('attribute_name') == utils.MAIL_GRDM:
                    # Get query from osf_user table instead
                    if len(include_osf_user_query_string) > 0:
                        include_osf_user_query_string += ' AND '
            ...
            # Get user list that met the logic condition use raw SQL with %s placeholder and params list to prevent SQL injection
