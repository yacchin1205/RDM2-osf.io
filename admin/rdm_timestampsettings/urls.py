from django.urls import re_path
from . import views


urlpatterns = [
    re_path(r'^$', views.InstitutionList.as_view(), name='institutions'),
    re_path(r'^force/(?P<institution_id>[0-9]+)/(?P<timestamp_pattern_division>[0-9])/(?P<forced>[01])$',
        views.InstitutionTimeStampPatternForce.as_view(), name='institutionsforce'),
    re_path(r'^(?P<institution_id>[0-9]+)/nodes/$', views.InstitutionNodeList.as_view(), name='nodes'),
    re_path(r'^(?P<institution_id>[0-9]+)/nodes/change/(?P<guid>[a-z0-9]+)/(?P<timestamp_pattern_division>[0-9])$',
        views.NodeTimeStampPatternChange.as_view(), name='nodeforce'),
]
