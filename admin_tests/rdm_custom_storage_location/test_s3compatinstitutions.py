...
class TestSaveCredentials(AdminTestCase):
    ...
    def test_success(self, mock_testconnection):
        ...
        assert external_account.provider == 's3compatinstitutions'
        assert external_account.provider_id == 's3.compat.co.jp\tNon-empty-access-key'
        assert external_account.oauth_secret == 's3.compat.co.jp'
        ...
        assert wb_settings['storage']['provider'] == 's3compatinstitutions'
...
class TestFetchCredentialsView(AdminTestCase):
    ...
    def test_post_default(self):
        ...
        assert response_body.get('s3compatinstitutions_access_key') == ''
        assert response_body.get('s3compatinstitutions_access_key') == ''
