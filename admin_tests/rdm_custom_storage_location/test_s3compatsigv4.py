...
class TestConnection(AdminTestCase):
    ...
    def test_empty_keys_with_provider(self):
        params = {
            's3compatsigv4_endpoint_url': '',
            's3compatsigv4_access_key': '',
            's3compatsigv4_secret_key': '',
    ...
    @mock.patch('addons.s3compatsigv4.views.utils.get_user_info', return_value=True)
    ...
    @mock.patch('addons.s3compatsigv4.views.utils.bucket_exists', return_value=False)
    @mock.patch('addons.s3compatsigv4.views.utils.can_list', return_value=True)
    @mock.patch('addons.s3compatsigv4.views.utils.get_user_info')
    ...
    @mock.patch('addons.s3compatsigv4.views.utils.bucket_exists', return_value=True)
    @mock.patch('addons.s3compatsigv4.views.utils.can_list', return_value=True)
    @mock.patch('addons.s3compatsigv4.views.utils.get_user_info')
    def test_success(self, mock_get_user_info, mock_can_list, mock_bucket_exists):
        mock_get_user_info.return_value = {'id': '12346789', 'display_name': 's3.user'}
    ...
    @mock.patch('addons.s3compatsigv4.views.utils.get_user_info', return_value=None)
...
class TestSaveCredentials(AdminTestCase):
    ...
    def test_provider_missing(self):
        response = self.view_post({
            ...
            's3compatsigv4_endpoint_url': 's3.compat.co.jp',
            's3compatsigv4_access_key': 'Non-empty-access-key',
            's3compatsigv4_secret_key': 'Non-empty-secret-key',
            's3compatsigv4_bucket': 'Cute bucket',
            's3compatsigv4_server_side_encryption': 'False',
        })
    ...
    @mock.patch('admin.rdm_custom_storage_location.utils.test_s3compatsigv4_connection')
    def test_success(self, mock_testconnection):
        ...
        nt.assert_equals(wb_settings['storage']['provider'], 's3compatsigv4')
    ...
    def test_invalid_credentials(self, mock_testconnection):
        ...
        response = self.view_post({
            'storage_name': 'My storage',
            's3compatsigv4_endpoint_url': 's3.compat.co.jp',
            's3compatsigv4_access_key': 'Wrong-access-key',
            's3compatsigv4_secret_key': 'Wrong-secret-key',
            's3compatsigv4_bucket': 'Cute bucket',
            's3compatsigv4_server_side_encryption': 'False',
            'provider_short_name': 's3compatsigv4',
        })
