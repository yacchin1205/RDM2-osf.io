...
class WorkflowEngineAdminTests(AdminTestCase):
    def setUp(self):
        ...
        self.base_url = reverse('rdm_workflow:engine-list')
    ...
    def test_super_admin_can_view_engine_list(self):
        request = self._request_with_messages('get', self.base_url, self.super_admin)
        response = views.WorkflowEngineListView.as_view()(request)
    ...
    def test_register_engine(self):
        request = self._request_with_messages('post', self.base_url, self.super_admin, self._create_engine_payload())
        response = views.WorkflowEngineListView.as_view()(request)
    ...
    def test_register_engine_with_keyset_import(self):
        ...
        with mock.patch('admin.rdm_workflow.views.import_gateway_public_keys', return_value=2) as importer:
            request = self._request_with_messages('post', self.base_url, self.super_admin, payload)
            response = views.WorkflowEngineListView.as_view()(request)
    ...
    def test_deactivate_engine_disables_keys(self):
        # Create engine first
        create_request = self._request_with_messages('post', self.base_url, self.super_admin, self._create_engine_payload())
        views.WorkflowEngineListView.as_view()(create_request)
        ...
        deactivate_request = self._request_with_messages('post', self.base_url, self.super_admin, {
            'action': 'deactivate_engine',
            'engine_id': engine.engine_id,
        })
        response = views.WorkflowEngineListView.as_view()(deactivate_request)
        self.assertEqual(response.status_code, 302)
        engine.refresh_from_db()
        self.assertFalse(engine.is_active)
        self.assertFalse(WorkflowEngineKey.objects.get(engine_id=engine.engine_id, kid='kidA').is_active)
    ...
    def test_activate_engine(self):
        ...
        views.WorkflowEngineListView.as_view()(deactivate_request)
        engine.refresh_from_db()
        self.assertFalse(engine.is_active)

        activate_request = self._request_with_messages('post', self.base_url, self.super_admin, {
            'action': 'activate_engine',
            'engine_id': engine.engine_id,
        })
        response = views.WorkflowEngineListView.as_view()(activate_request)
        self.assertEqual(response.status_code, 302)
        engine.refresh_from_db()
    ...
    def test_manage_keys_add_and_deactivate(self):
        ...
        keys_url = reverse('rdm_workflow:engine-keys', kwargs={'engine_id': engine.engine_id})
        ...
        response = views.WorkflowEngineKeyView.as_view()(add_request, engine_id=engine.engine_id)
        self.assertEqual(response.status_code, 302)
        ...
        response = views.WorkflowEngineKeyView.as_view()(deactivate_request, engine_id=engine.engine_id)
        self.assertEqual(response.status_code, 302)
        key.refresh_from_db()
