...
class TestInstitutionDelete(AdminTestCase):
    ...
    def test_cannot_delete_if_nodes_affiliated(self):
        ...
        redirect = self.view.delete(self.request)
        assert (redirect.url) == ('/institutions/{}/cannot_delete/'.format(self.institution.id))
        assert (redirect.status_code) == (302)
...
class TestGetUserListWithQuota(AdminTestCase):
    ...
    def test_used_quota_bytes(self):
        ...
        assert (user_quota['usage']) == (560)
        assert (round(user_quota['usage_value'], 1)) == (0.6)
        assert (user_quota['usage_abbr']) == ('KB')

        assert (user_quota['remaining']) == (int(100 * api_settings.SIZE_UNIT_GB) - 560)
        assert (round(user_quota['remaining_value'], 1)) == (100)
        assert (user_quota['remaining_abbr']) == ('GB')

        assert (round(user_quota['ratio'], 1)) == (0)
    ...
    def test_used_quota_giga(self):
        ...
        assert (user_quota['usage']) == (used)
        assert (round(user_quota['usage_value'], 1)) == (5.2)
        assert (user_quota['usage_abbr']) == ('GB')

        assert (user_quota['remaining']) == (100 * api_settings.SIZE_UNIT_GB - used)
        assert (round(user_quota['remaining_value'], 1)) == (100 - 5.2)
        assert (user_quota['remaining_abbr']) == ('GB')

        assert (round(user_quota['ratio'], 1)) == (5.2)
...
class TestQuotaUserList(AdminTestCase):
    ...
    def test_get_user_quota_info_max_quota_zero(self):
        ...
        assert (response['ratio']) is not None
        assert (response['ratio']) == (100)
...
class TestRecalculateQuota(AdminTestCase):
    ...
    def test_post_method_with_institution_id(self, mock_osfuser, mock_update_user_used_quota_method):
        ...
        assert (response.status_code) == (302)
        assert (response.url) == (reverse('institutions:institution_user_list', kwargs={'institution_id': self.institution1.id}))
        mock_osfuser.filter.assert_called()
        mock_update_user_used_quota_method.assert_called()
