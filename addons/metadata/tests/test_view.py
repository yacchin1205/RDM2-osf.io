...
class TestViews(BaseAddonTestCase, OsfTestCase):
    ...
    def test_single_file_metadata(self):
        ...
        url = self.project.api_url_for('{}_get_project'.format(SHORT_NAME))
        res = self.app.get(url, auth=self.user.auth)
        assert res.json['data']['attributes']['files'] == [{'path': 'osfstorage/', 'generated': False, 'hash': '1234567890', 'urlpath': '/{}/files/dir/osfstorage/'.format(self.node_settings.owner._id), 'folder': True, 'items': [{'active': True, 'schema': 'xxxx', 'data': {'test': True}}]}]
...
class TestSuggestionsViews(BaseAddonTestCase, OsfTestCase):
    ...
    def test_dir_with_multiple_keys(self, mock_get_metadata_assets):
        ...
        assert res.status_code == http_status.HTTP_200_OK
        assert res.json == {'data': {'id': self.project._id, 'type': 'file-metadata-suggestion', 'attributes': {'filepath': 'dir/osfstorage/dir1/', 'suggestions': [{'key': 'file-data-number', 'value': 'files/dir/osfstorage/dir1/'}, {'key': 'asset:title', 'value': {'title': 'apple'}}, {'key': 'asset:title', 'value': {'title': 'pine'}}, {'key': 'asset:title', 'value': {'title': 'pineapple'}}]}}}
    ...
    def test_file_with_multiple_keys(self, mock_get_metadata_assets):
        ...
        with mock.patch.object(BaseFileNode, 'resolve_class', return_value=mock_resolved_class):
            ...
            res = self.app.get(url, auth=self.user.auth, params={'key[]': ['file-data-number', 'asset:title']})
            assert res.status_code == http_status.HTTP_200_OK
            assert res.json == {'data': {'id': self.project._id, 'type': 'file-metadata-suggestion', 'attributes': {'filepath': filepath, 'suggestions': [{'key': 'file-data-number', 'value': filepath_guid}, {'key': 'asset:title', 'value': {'title': 'apple'}}, {'key': 'asset:title', 'value': {'title': 'pine'}}, {'key': 'asset:title', 'value': {'title': 'pineapple'}}]}}}
    ...
    def test_asset_title_with_keyword(self, mock_get_metadata_assets):
        ...
        assert res.status_code == http_status.HTTP_200_OK
        assert res.json == {'data': {'id': self.project._id, 'type': 'file-metadata-suggestion', 'attributes': {'filepath': filepath, 'suggestions': [{'key': 'asset:title', 'value': {'title': 'apple'}}, {'key': 'asset:title', 'value': {'title': 'pineapple'}}]}}}
