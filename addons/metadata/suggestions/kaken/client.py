...
class ResourceSyncClient:
    ...
    def process_resource_list(self, url: str) -> Iterator[str]:
        """
    ...
    def process_change_list(self, url: str, since: datetime = None) -> Iterator[Tuple[str, str, datetime]]:
        """
        ...
        for url_elem in root.findall('.//{http://www.sitemaps.org/schemas/sitemap/0.9}url'):
            ...
            if loc is not None and loc.text.endswith('.json'):
                ...
                if since and lastmod_dt and lastmod_dt <= since:
                    continue

                # Determine action based on ResourceSync extension
                action = 'updated'  # Default action
                for link in url_elem.findall('.//{http://www.w3.org/2005/Atom}link'):
                    if link.get('rel') == 'related':
                        if 'created' in link.get('href', ''):
                            action = 'created'
                        elif 'deleted' in link.get('href', ''):
                            action = 'deleted'
                        break
    ...
    def fetch_researcher_data(self, json_url: str) -> Dict:
        ...
        try:
            ...
            logger.exception(f"Failed to parse JSON from {json_url}: {e}")
            raise
