...
class WorkflowEngineViewTests(OsfTestCase):
    ...
    @staticmethod
    def _create_engine(owner=None) -> WorkflowEngine:
    ...
    @staticmethod
    def _registration_url(node) -> str:
        return api_url_for('upsert_registration', pid=node._id)
    ...
    @staticmethod
    def _activation_url(route: str, node, registration) -> str:
        registration_id = registration._id if hasattr(registration, '_id') else registration
        return api_url_for(route, pid=node._id, registration_id=registration_id)

    @staticmethod
    def _run_url(node, registration) -> str:
        registration_id = registration._id if hasattr(registration, '_id') else registration
        return api_url_for('start_run', pid=node._id, registration_id=registration_id)
    ...
    def test_register_workflow_success(self):
        ...
        assert data['activation_token_settings'] == token_settings
        assert data['activation_activated_by'] == owner._id
        assert WorkflowRegistration.objects.filter(node=node, definition__definition_id=definition_id).exists()
    ...
    def test_register_workflow_reuses_existing_registration(self):
        owner = AuthUserFactory()
        node = self._create_project_with_workflow(owner)
        engine = self._create_engine(owner=owner)
        ...
        # First call creates the registration
        self.app.post_json(
            self._registration_url(node),
            {
                'engine_id': engine.engine_id,
    ...
    def test_list_registrations_includes_local_and_shared(self):
        owner = AuthUserFactory()
        node = self._create_project_with_workflow(owner)
        engine = self._create_engine(owner=owner)
        ...
        self.app.post_json(
            self._registration_url(shared_node),
            {
        ...
        self.app.post_json(
            self._registration_url(hidden_node),
            {
        ...
        response = self.app.get(
            api_url_for('list_registrations', pid=node._id),
            auth=owner.auth,
        )
        assert response.status_code == http_status.HTTP_200_OK
        registrations = response.json['data']
        guids = {entry['definition_id']: entry for entry in registrations}
        ...
        assert guids[definition_id_local]['is_enabled'] is True
        assert guids[definition_id_shared]['is_enabled'] is False
        assert guids[definition_id_local]['activation_id']
    ...
    def test_activation_lifecycle_for_shared_registration(self):
        owner = AuthUserFactory()
        node = self._create_project_with_workflow(owner)
        ...
        registration = WorkflowRegistration.objects.get(
            node=shared_node,
        ...
        response = self.app.put_json(
            self._activation_url('upsert_activation', node, registration),
            activation_payload,
            auth=owner.auth,
        )
        assert response.status_code == http_status.HTTP_201_CREATED
        ...
        response = self.app.get(
            self._activation_url('retrieve_activation', node, registration),
            auth=owner.auth,
        )
        assert response.status_code == http_status.HTTP_200_OK
        ...
        response = self.app.delete(
            self._activation_url('deactivate_activation', node, registration),
            auth=owner.auth,
        )
    ...
    def test_start_run_enqueues_celery_job(self, mock_task, mock_enqueue):
        ...
        response = self.app.post_json(
            self._run_url(node, registration),
            payload,
            auth=owner.auth,
        )
