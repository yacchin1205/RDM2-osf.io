...
function buildDisplayLabel(registration) {
    var parts = [];
    if (registration.label) {
        parts.push(registration.label);
    } else if (registration.definition_name) {
        parts.push(registration.definition_name);
    } else if (registration.definition_key) {
        parts.push(registration.definition_key);
    } else if (registration.definition_id) {
        parts.push(registration.definition_id);
    }
    if (!registration.is_local && registration.node_title) {
        parts.push('[' + registration.node_title + ']');
    }
    return parts.join(' ');
}
...
function WorkflowWidgetViewModel() {
    ...
    self.loadingRegistrations = ko.observable(true);
    ...
    self.launchUrlFor = function(registrationId) {
        ...
        return self.pageUrl + '#start=' + encodeURIComponent(registrationId);
    };
    ...
    self.selectedRegistrationId = ko.observable('');

    self.selectedRegistration = ko.pureComputed(function() {
        var currentId = self.selectedRegistrationId();
        if (!currentId) {
            return null;
        }
        return self.activeRegistrations().find(function(entry) {
            return entry.id === currentId;
        }) || null;
    });

    self.selectedRegistrationLabel = ko.pureComputed(function() {
        var selection = self.selectedRegistration();
        return selection ? selection.displayLabel : self.selectWorkflowPrompt;
    });

    self.selectedRegistrationUrl = ko.pureComputed(function() {
        var selection = self.selectedRegistration();
        return selection ? self.launchUrlFor(selection.id) : '#';
    });

    self.selectRegistration = function(entry) {
        if (entry && entry.id) {
            self.selectedRegistrationId(entry.id);
        }
        return true;
    };

    self.ensureSelectedRegistration = function() {
        var active = self.activeRegistrations();
        if (!active.length) {
            self.selectedRegistrationId('');
            return;
        }
        var currentId = self.selectedRegistrationId();
        ...
        if (!hasCurrent) {
            self.selectedRegistrationId(active[0].id);
        }
    };
    ...
    self.fetchRegistrations = function() {
        if (!self.apiBaseUrl) {
            ...
            self.loadingRegistrations(false);
        ...
        request.done(function(response) {
            ...
            self.ensureSelectedRegistration();
        });
