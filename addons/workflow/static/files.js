...
function WorkflowButtons() {
  ...
  self.loadWorkflowsForFile = function(nodeId, filepath, schemaName, callback) {
    ...
    if (self.workflowRegistrations[cacheKey]) {
      callback(self.workflowRegistrations[cacheKey]);
      return;
    }
    ...
    $.ajax({
      ...
      const workflows = response.data.map(function(activation) {
        const registration = activation.registration;
        ...
        return {
          ...
          fields: registration.definition_form_schema.fields
        };
      });
      ...
      self.workflowRegistrations[cacheKey] = filtered;
      callback(filtered);
  ...
  self.createWorkflowButton = function(filepath, item, schemaName, createButton) {
    ...
    const workflows = self.workflowRegistrations[filepath] || [];
