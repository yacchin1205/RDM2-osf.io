...
class NodeSettings(BaseNodeSettings):
    ...
    def complete(self) -> bool:
        # Workflow add-on is considered configured once installed; registrations are optional per use-case.
        return True

    @property
    def active_registrations(self):
        registration_model = apps.get_model('addons_workflow', 'WorkflowRegistration')
        if not self.owner_id:
            return registration_model.objects.none()
...
class WorkflowRegistration(BaseModel):
    ...
    node = models.ForeignKey(
        ...
        related_name='workflow_registrations',
    )
    ...
    def __str__(self) -> str:  # pragma: no cover
        ...
        return f"WorkflowRegistration(node={self.node_id}, definition={definition_id})"
    ...
    def load(cls, identifier):
        if isinstance(identifier, int):
            ...
            if identifier.isdigit():
                ...
                raise ValueError('WorkflowRegistration identifier must be a base-10 integer string')
        else:
            raise TypeError('WorkflowRegistration identifier must be an int or numeric string')
        try:
...
class WorkflowActivation(BaseModel):
    """Enablement of a shared workflow registration on a specific project."""
    ...
    registration = models.ForeignKey(
        'addons_workflow.WorkflowRegistration',
        on_delete=models.CASCADE,
    ...
    class Meta:
        unique_together = ('node', 'registration')
        ordering = ('node', 'registration')
    ...
    def __str__(self) -> str:  # pragma: no cover
        return f"WorkflowActivation(node={self.node_id}, registration={self.registration_id})"
