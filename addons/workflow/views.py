...
def _get_engine_or_404(engine_id: str, user) -> WorkflowEngine:
    ...
    if engine.created_by_id != user.id:
        has_institution_access = (
            bool(user_institution_ids) and engine.created_by and engine.created_by.affiliated_institutions.filter(id__in=user_institution_ids).exists()
        )
        ...
        if not has_institution_access:
            ...
            has_registration_access = WorkflowRegistration.objects.filter(
        ...
        if not has_institution_access and not has_registration_access:
            logger.info(
                'Engine lookup denied: user=%s lacks access to engine=%s (institution=%s registration=%s)',
                user._id,
                engine.engine_id,
                has_institution_access,
...
def _get_registration_or_404(registration_id: str, user) -> WorkflowRegistration:
    if not user:
        raise HTTPError(
            http_status.HTTP_404_NOT_FOUND,
    ...
    registration = WorkflowRegistration.load(registration_id)
    ...
    if registration.node.is_deleted:
        raise HTTPError(
            http_status.HTTP_404_NOT_FOUND,
            data={'message': 'Workflow registration not found.'},
        )
...
def _serialize_registration(
    ...
    *,
    ...
    return {
        'id': registration._id,
        'node_id': registration.node._id if registration.node_id else None,
        'node_title': registration.node.title if registration.node_id else None,
        'is_local': (registration.node_id == current_node_id) if current_node_id is not None else None,
        'engine_id': registration.engine_id,
        'definition_id': registration.process_definition_id,
        'definition_key': registration.definition_key,
        'definition_name': registration.definition_name,
        'definition_version': registration.definition_version,
        'definition_category': registration.definition_category,
        'definition_description': registration.definition_description,
        'definition_deployment_id': registration.definition_deployment_id,
        'definition_form_schema': registration.definition_form_schema,
        'token_settings': registration.token_settings,
...
@must_have_addon(SHORT_NAME, 'node')
def upsert_registration(auth, **kwargs):
    ...
    if request.files and 'workflow_zip' in request.files:
        ...
        if node._id not in engine.upload_whitelist_node_ids:
            raise HTTPError(
                http_status.HTTP_403_FORBIDDEN,
    ...
    registration, created = upsert_workflow_registration(
        node,
        engine_id=engine.engine_id,
        definition_id=definition_id,
...
@must_have_addon(SHORT_NAME, 'node')
def list_registrations(auth, **kwargs):
    node = kwargs.get('node') or kwargs['project']
    user = auth.user
    ...
    for registration in registrations:
        ...
        combined.append(registration)

    activation_map: Dict[int, WorkflowActivation] = {}
    if combined:
        activation_map = {
            activation.registration_id: activation
            for activation in WorkflowActivation.objects.filter(
                node=node,
                registration__in=[reg.id for reg in combined],
            ).select_related('registration', 'activated_by')
        }
...
@must_have_permission('write')
@must_have_addon(SHORT_NAME, 'node')
def update_registration(auth, registration_id: str, **kwargs):
    node = kwargs.get('node') or kwargs['project']
    user = auth.user

    registration = _get_registration_or_404(registration_id, user)

    if registration.node_id != node.id:
        raise HTTPError(
            http_status.HTTP_403_FORBIDDEN,
            data={'message': 'Cannot update registration from a different project.'},
        )
...
@must_have_permission('write')
@must_have_addon(SHORT_NAME, 'node')
def delete_registration(auth, registration_id: str, **kwargs):
    node = kwargs.get('node') or kwargs['project']
    user = auth.user
    ...
    if registration.node_id != node.id:
        raise HTTPError(
            http_status.HTTP_403_FORBIDDEN,
            data={'message': 'Cannot delete registration from a different project.'},
        )

    if registration.is_active:
        raise HTTPError(
            http_status.HTTP_400_BAD_REQUEST,
            data={'message': 'Cannot delete active registration. Disable it first.'},
        )
...
@must_have_permission('write')
@must_have_addon(SHORT_NAME, 'node')
def retrieve_activation(auth, registration_id: str, **kwargs):
    node = kwargs.get('node') or kwargs['project']
    user = auth.user
    ...
    try:
        activation = WorkflowActivation.objects.select_related('registration', 'node', 'activated_by').get(
            node=node,
            registration=registration,
        )
...
@must_have_permission('write')
@must_have_addon(SHORT_NAME, 'node')
def upsert_activation(auth, registration_id: str, **kwargs):
    node = kwargs.get('node') or kwargs['project']
    user = auth.user
...
@must_have_permission('write')
@must_have_addon(SHORT_NAME, 'node')
def deactivate_activation(auth, registration_id: str, **kwargs):
    node = kwargs.get('node') or kwargs['project']
    user = auth.user
    ...
    try:
        activation = WorkflowActivation.objects.get(node=node, registration=registration)
    except WorkflowActivation.DoesNotExist as error:
        raise HTTPError(
            http_status.HTTP_404_NOT_FOUND,
            data={'message': 'Workflow activation not found.'},
...
@must_have_permission('write')
@must_have_addon(SHORT_NAME, 'node')
def start_run(auth, registration_id: str, **kwargs):
    node = kwargs.get('node') or kwargs['project']
    user = auth.user
