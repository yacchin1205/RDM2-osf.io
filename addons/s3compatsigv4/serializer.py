...
class S3CompatSigV4Serializer(StorageAddonSerializer):
    @property
    ...
    def addon_serialized_urls(self):
        ...
        result = {
            'accounts': node.api_url_for('s3compatsigv4_account_list'),
            'createBucket': node.api_url_for('s3compatsigv4_create_bucket'),
            'importAuth': node.api_url_for('s3compatsigv4_import_auth'),
            'create': node.api_url_for('s3compatsigv4_add_user_account'),
            'deauthorize': node.api_url_for('s3compatsigv4_deauthorize_node'),
            'folders': node.api_url_for('s3compatsigv4_folder_list'),
            'config': node.api_url_for('s3compatsigv4_set_config'),
            'files': node.web_url_for('collect_file_trees'),
            'attachedService': node.api_url_for('s3compatsigv4_attached_service'),
        }
        if user_settings:
