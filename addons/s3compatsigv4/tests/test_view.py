...
from addons.s3compatsigv4.tests.utils import S3CompatSigV4AddonTestCase
from addons.s3compatsigv4.utils import validate_bucket_name
...
class Tests3compatsigv4Views(S3CompatSigV4AddonTestCase, OAuthAddonConfigViewsTestCaseMixin, OsfTestCase):
    def setUp(self):
        self.mock_can_list = mock.patch('addons.s3compatsigv4.views.utils.can_list')
        self.mock_can_list.return_value = True
        self.mock_can_list.start()
        self.mock_uid = mock.patch('addons.s3compatsigv4.views.utils.get_user_info')
        self.mock_uid.return_value = {'id': '1234567890', 'display_name': 's3compatsigv4.user'}
        self.mock_uid.start()
        self.mock_exists = mock.patch('addons.s3compatsigv4.views.utils.bucket_exists')
        self.mock_exists.return_value = True
        self.mock_exists.start()
        super(Tests3compatsigv4Views, self).setUp()

    def tearDown(self):
        self.mock_can_list.stop()
        self.mock_uid.stop()
        self.mock_exists.stop()
        super(Tests3compatsigv4Views, self).tearDown()
    ...
    def test_s3compatsigv4_set_bucket_no_auth(self):
    ...
    def test_s3compatsigv4_set_bucket_registered(self):
    ...
    def test_s3compatsigv4_get_node_settings_owner(self):
    ...
    @mock.patch('addons.s3compatsigv4.models.get_bucket_names')
    def test_folder_list(self, mock_names):
        mock_names.return_value = ['bucket1', 'bucket2']
        super(Tests3compatsigv4Views, self).test_folder_list()

    @mock.patch('addons.s3compatsigv4.models.bucket_exists')
    @mock.patch('addons.s3compatsigv4.models.get_bucket_location_or_error')
    @mock.patch('addons.s3compatsigv4.models.find_service_by_host')
...
class TestCreateBucket(S3CompatSigV4AddonTestCase, OsfTestCase):
    ...
    def setUp(self):
        ...
        self.project.add_addon('s3compatsigv4', auth=self.consolidated_auth)
        self.project.creator.add_addon('s3compatsigv4')

        self.user_settings = self.user.get_addon('s3compatsigv4')
        self.user_settings.access_key = 'We-Will-Rock-You'
        self.user_settings.secret_key = 'Idontknowanyqueensongs'
        self.user_settings.save()

        self.node_settings = self.project.get_addon('s3compatsigv4')
        self.node_settings.bucket = 'Sheer-Heart-Attack'
        self.node_settings.user_settings = self.project.creator.get_addon('s3compatsigv4')
    ...
    @mock.patch('addons.s3compatsigv4.views.utils.get_bucket_names')
    ...
    def test_create_bucket_fail(self, mock_make):
        error = ClientError(
            {'Error': {'Code': '418', 'Message': 'This should work'}},
            'CreateBucket'
        )
