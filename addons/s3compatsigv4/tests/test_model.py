...
from addons.s3compatsigv4.models import NodeSettings
from addons.s3compatsigv4.tests.factories import (
    S3CompatSigV4UserSettingsFactory,
    S3CompatSigV4NodeSettingsFactory,
...
class TestUserSettings(OAuthAddonUserSettingTestSuiteMixin, unittest.TestCase):
    ...
    full_name = 'S3 Compatible Storage (SigV4)'
...
class TestNodeSettings(OAuthAddonNodeSettingsTestSuiteMixin, unittest.TestCase):
    ...
    full_name = 'S3 Compatible Storage (SigV4)'
    ExternalAccountFactory = S3CompatSigV4AccountFactory
    NodeSettingsFactory = S3CompatSigV4NodeSettingsFactory
    NodeSettingsClass = NodeSettings
    UserSettingsFactory = S3CompatSigV4UserSettingsFactory
    ...
    def test_does_not_get_copied_to_registrations(self, mock_archive):
        ...
        assert not (registration.has_addon('s3compatsigv4'))
    ...
    def test_set_folder(self, mock_service, mock_location, mock_exists):
        ...
        assert (self.node_settings.folder_name) == ('{} (Default)'.format(folder_id))
        assert (self.node_settings.folder_location) == ('')
        # Log was saved
    ...
    def test_set_folder_undefined_location(self, mock_service, mock_location, mock_exists):
        ...
        # Bucket was set
        assert (self.node_settings.folder_id) == (folder_id)
        assert (self.node_settings.folder_name) == ('{} (dummy-1)'.format(folder_id))
        assert (self.node_settings.folder_location) == ('dummy-1')
        # Log was saved
        last_log = self.node.logs.latest()
        assert (last_log.action) == ('{0}_bucket_linked'.format(self.short_name))
    ...
    def test_set_folder_defined_location(self, mock_service, mock_location, mock_exists):
        ...
        # Bucket was set
        assert (self.node_settings.folder_id) == (folder_id)
        assert (self.node_settings.folder_name) == ('{} (Location2)'.format(folder_id))
        assert (self.node_settings.folder_location) == ('dummy-2')
        # Log was saved
        last_log = self.node.logs.latest()
        assert (last_log.action) == ('{0}_bucket_linked'.format(self.short_name))
    ...
    def test_set_folder_encrypt_uploads_with_encryption_setting(self, mock_service, mock_location, mock_exists):
        ...
        self.node_settings.save()
        # encrypt_uploads set
        assert (self.node_settings.encrypt_uploads) == (False)
    ...
    def test_set_folder_encrypt_uploads_without_encryption_setting(self, mock_service, mock_location, mock_exists):
        ...
        self.node_settings.save()
        # encrypt_uploads set
        assert (self.node_settings.encrypt_uploads) == (True)
    ...
    @mock.patch('addons.s3compatsigv4.models.get_bucket_location_or_error')
    @mock.patch('addons.s3compatsigv4.models.find_service_by_host')
    def test_serialize_settings(self, mock_service, mock_location, mock_exists):
        mock_exists.return_value = True
        mock_location.return_value = 'us-east-1'
        mock_service.return_value = {'name': 'Dummy',
                                     'host': 'dummy.example.com',
                                     'bucketLocations': {'us-east-1': {'name': 'Fukushima'}}}
        folder_id = '1234567890'
        self.node_settings.set_folder(folder_id, auth=Auth(self.user))
        self.node_settings.save()
