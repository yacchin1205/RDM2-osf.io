...
class S3CompatSigV4Folder(S3CompatSigV4FileNode, Folder):
    pass


class S3CompatSigV4File(S3CompatSigV4FileNode, File):
    version_identifier = 'version'


class UserSettings(BaseOAuthUserSettings):
    oauth_provider = S3CompatSigV4Provider
...
class NodeSettings(BaseOAuthNodeSettings, BaseStorageAddon):
    ...
    def serialize_waterbutler_credentials(self):
        if not self.has_auth:
            raise exceptions.AddonError('Cannot serialize credentials for S3 Compatible Storage (SigV4) addon')
        host = self.external_account.provider_id.split('\t')[0]
    ...
    def serialize_waterbutler_settings(self):
        if not self.folder_id:
            raise exceptions.AddonError('Cannot serialize settings for S3 Compatible Storage (SigV4) addon')
        return {
            'bucket': self.folder_id,
            'region': self.folder_location,
            'encrypt_uploads': self.encrypt_uploads
        }

    def create_waterbutler_log(self, auth, action, metadata):
        url = self.owner.web_url_for('addon_view_or_download_file', path=metadata['path'], provider='s3compatsigv4')

        self.owner.add_log(
            's3compatsigv4_{0}'.format(action),
            auth=auth,
            params={
