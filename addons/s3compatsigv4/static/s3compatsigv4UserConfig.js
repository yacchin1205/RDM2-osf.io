/**
* Module that controls the S3 Compatible Storage (SigV4) user settings. Includes Knockout view-model
* for syncing data.
*/
...
var language = require('js/osfLanguage').Addons.s3compatsigv4;
var osfHelpers = require('js/osfHelpers');
var addonSettings = require('js/addonSettings');
var ChangeMessageMixin = require('js/changeMessage');

var s3compatsigv4Settings = require('json-loader!./settings.json');

var ExternalAccount = addonSettings.ExternalAccount;

var $modal = $('#s3compatsigv4InputCredentials');


function ViewModel(url) {
    var self = this;

    self.properName = 'S3 Compatible Storage (SigV4)';
    ...
    self.account_url = '/api/v1/settings/s3compatsigv4/accounts/';
    self.accounts = ko.observableArray();

    ChangeMessageMixin.call(self);

    /** Reset all fields from S3 Compatible Storage (SigV4) credentials input modal */
    self.clearModal = function() {
        self.message('');
        self.messageClass('text-info');
        self.selectedService(s3compatsigv4Settings['availableServices'][0]);
        self.accessKey(null);
        self.secretKey(null);
    };
    /** Send POST request to authorize S3 Compatible Storage (SigV4) */
    self.connectAccount = function() {
        ...
        return osfHelpers.postJSON(
            ...
            Raven.captureMessage('Could not authenticate with S3 Compatible Storage (SigV4)', {
                extra: {
    ...
    self.updateAccounts = function() {
        return $.ajax({
            ...
            $('#s3compatsigv4-header').osfToggleHeight({height: 160});
    ...
    self.askDisconnect = function(account) {
        var self = this;
        bootbox.confirm({
            title: 'Disconnect S3 Compatible Storage (SigV4) Account?',
            message: '<p class="overflow">' +
                'Are you sure you want to disconnect the S3 Compatible Storage (SigV4) account <strong>' +
                osfHelpers.htmlEscape(account.name) + '</strong>? This will revoke access to S3 Compatible Storage (SigV4) for all projects associated with this account.' +
                '</p>',
...
function s3compatsigv4UserConfig(selector, url) {
    // Initialization code
    var self = this;
...
module.exports = {
    s3compatsigv4ViewModel: ViewModel,
    s3compatsigv4UserConfig: s3compatsigv4UserConfig
};
