...
var s3compatsigv4FolderPickerViewModel = oop.extend(OauthAddonFolderPicker, {
    ...
    connectAccount: function() {
        ...
        return $osf.postJSON(
            ...
            $osf.unblock();
            self.clearModal();
            $('#s3compatsigv4InputCredentials').modal('hide');
            self.changeMessage('Successfully added S3 Compatible Storage (SigV4) credentials.', 'text-success', null, true);
            ...
            Raven.captureMessage('Could not add S3 Compatible Storage (SigV4) credentials', {
                extra: {
    ...
    /**
     * Tests if the given string is a valid S3 Compatible Storage (SigV4) bucket name.  Supports two modes: strict and lax.
                    ...
                    callback: function () {
                        ...
                        if (!bucketName) {
                            ...
                            bootbox.confirm({
                                ...
                                message: 'S3 Compatible Storage (SigV4) buckets can contain lowercase letters, numbers, and hyphens separated by' +
...
// Public API
function s3compatsigv4NodeConfig(addonName, selector, url, folderPicker, opts, tbOpts) {
    var self = this;
    self.url = url;
    self.folderPicker = folderPicker;
    opts = opts || {};
    tbOpts = tbOpts || {};
    self.viewModel = new s3compatsigv4FolderPickerViewModel(addonName, url, selector, folderPicker, opts, tbOpts);
    self.viewModel.updateFromData();
    $osf.applyBindings(self.viewModel, selector);
}

module.exports = {
    s3compatsigv4NodeConfig: s3compatsigv4NodeConfig,
    _s3compatsigv4NodeConfigViewModel: s3compatsigv4FolderPickerViewModel
};
