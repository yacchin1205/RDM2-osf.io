...
class OAuthAddonUserSettingTestSuiteMixin(OAuthAddonModelTestSuiteMixinBase):
    ...
    def test_verify_oauth_access_no_metadata(self):
        ...
        assert self.user_settings.verify_oauth_access(node=self.node, external_account=self.external_account)

        assert not self.user_settings.verify_oauth_access(node=self.node, external_account=self.ExternalAccountFactory())
    ...
    def test_verify_oauth_access_metadata(self):
        ...
        assert self.user_settings.verify_oauth_access(node=self.node, external_account=self.external_account, metadata={'folder': 'fake_folder_id'})

        assert not self.user_settings.verify_oauth_access(node=self.node, external_account=self.external_account, metadata={'folder': 'another_folder_id'})
...
class OAuthAddonNodeSettingsTestSuiteMixin(OAuthAddonModelTestSuiteMixinBase):
    ...
    def test_deauthorize(self):
        ...
        assert last_log.action == '{0}_node_deauthorized'.format(self.short_name)
    ...
    def test_set_user_auth(self):
        ...
        assert last_log.action == '{0}_node_authorized'.format(self.short_name)
...
class OAuthCitationsNodeSettingsTestSuiteMixin(
    ...
    def test_set_folder(self):
        ...
        assert self.user_settings.verify_oauth_access(node=self.node, external_account=self.external_account, metadata={'folder': 'fake-folder-id'})

        log = self.node.logs.latest()
        assert log.action == '{}_folder_selected'.format(self.short_name)
