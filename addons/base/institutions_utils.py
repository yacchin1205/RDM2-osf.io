...
class InstitutionsNodeSettings(BaseNodeSettings):
    ...
    def _institutions_disabled(self):
        ...
        in_institutions, _, region_provider = get_region_provider(self.owner)
        if not in_institutions:
            return False
...
def get_region_provider(node):
    ...
    if not region or not region.waterbutler_settings:
        return False, False, None # No valid regions
    region_id = region._id
    if not Institution.objects.filter(_id=region_id).exists():
        return False, False, None # No regions for institutional storage
    ...
    return True, region_disabled, region_provider
