        ...
        if isinstance(entity, list):
            for e in entity:
                if '@id' not in e and '@type' not in e:
                    logger.warning(f'No @id or @type: {e}, because root object must have @id and @type. Skip object {key}.')
                    continue
                if '@id' not in e:
            ...
            continue
        if '@id' not in entity:
            logger.warning(f'No @id: {entity}, because root object must have @id. Skip object {key}.')
            continue
...
def _flatten_json_ld(object, counts):
    ...
    for key, value in object.items():
        ...
        if isinstance(value, dict):
            if '@id' in value:
                raise ValueError(f'Unexpected @id: {value} for {key}')
            if '@type' not in value:
                raise ValueError(f'No @type: {value} for {key}')
            new_value = {}
        ...
        if isinstance(value, list):
            ...
            for v in value:
                if not isinstance(v, dict):
                    raise ValueError(f'Unexpected value: {v} for {key}')
                if _is_reference(value) or _is_literal(value):
                    new_value.push(value)
                    continue
                if '@id' in v:
                    raise ValueError(f'Unexpected @id: {v} for {key}')
                if '@type' not in v:
                    raise ValueError(f'No @type: {v} for {key}')
                entity_id = _generate_json_ld_id(v, counts)
                new_v = {}
                new_v.update(v)
                new_v['@id'] = entity_id
