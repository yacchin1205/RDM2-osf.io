...
class OneDriveBusinessConnection(S3Connection):
    def __init__(self, aws_access_key_id=None, aws_secret_access_key=None,
        ...
        super(OneDriveBusinessConnection, self).__init__(aws_access_key_id,
...
def connect_onedrivebusiness(host=None, access_key=None, secret_key=None, node_settings=None):
    """Helper to build an OneDriveBusinessConnection object
    """
    if host is None:
        host = settings.HOST
    ...
    if m is not None:
        host = m.group(1)
        port = int(m.group(2))
    return OneDriveBusinessConnection(access_key, secret_key,
...
def get_bucket_names(node_settings):
    try:
        buckets = connect_onedrivebusiness(node_settings=node_settings).get_all_buckets()
...
def create_bucket(node_settings, bucket_name):
    return connect_onedrivebusiness(node_settings=node_settings).create_bucket(bucket_name)
...
def bucket_exists(host, access_key, secret_key, bucket_name):
    ...
    connection = connect_onedrivebusiness(host, access_key, secret_key)
    ...
    try:
        ...
        connect_onedrivebusiness(host, access_key, secret_key).head_bucket(bucket_name)
...
def can_list(host, access_key, secret_key):
    ...
    try:
        connect_onedrivebusiness(host, access_key, secret_key).get_all_buckets()
...
def get_user_info(host, access_key, secret_key):
    ...
    try:
        return connect_onedrivebusiness(host, access_key, secret_key).get_all_buckets().owner
    except exception.S3ResponseError:
        return None
