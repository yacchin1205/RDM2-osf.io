...
from addons.onedrivebusiness.utils import validate_bucket_name
...
class TestOneDriveBusinessViews(OneDriveBusinessAddonTestCase, OAuthAddonConfigViewsTestCaseMixin, OsfTestCase):
    def setUp(self):
        self.mock_can_list = mock.patch('addons.onedrivebusiness.views.utils.can_list')
        self.mock_can_list.return_value = True
        self.mock_can_list.start()
        self.mock_uid = mock.patch('addons.onedrivebusiness.views.utils.get_user_info')
        self.mock_uid.return_value = {'id': '1234567890', 'display_name': 'onedrivebusiness.user'}
        self.mock_uid.start()
        self.mock_exists = mock.patch('addons.onedrivebusiness.views.utils.bucket_exists')
    ...
    def test_onedrivebusiness_settings_input_empty_keys(self):
        url = self.project.api_url_for('onedrivebusiness_add_user_account')
    ...
    def test_onedrivebusiness_settings_input_empty_access_key(self):
        url = self.project.api_url_for('onedrivebusiness_add_user_account')
        rv = self.app.post_json(url, {
            'access_key': '',
    ...
    def test_onedrivebusiness_settings_input_empty_secret_key(self):
        url = self.project.api_url_for('onedrivebusiness_add_user_account')
        rv = self.app.post_json(url, {
    ...
    def test_onedrivebusiness_settings_rdm_addons_denied(self):
    ...
    def test_onedrivebusiness_set_bucket_no_settings(self):
        user = AuthUserFactory()
        self.project.add_contributor(user, save=True)
        url = self.project.api_url_for('onedrivebusiness_set_config')
        res = self.app.put_json(
            url, {'onedrivebusiness_bucket': 'hammertofall'}, auth=user.auth,
            expect_errors=True
        )
        assert_equal(res.status_code, http_status.HTTP_400_BAD_REQUEST)

    def test_onedrivebusiness_set_bucket_no_auth(self):

        user = AuthUserFactory()
        user.add_addon('onedrivebusiness')
        self.project.add_contributor(user, save=True)
        url = self.project.api_url_for('onedrivebusiness_set_config')
        res = self.app.put_json(
            url, {'onedrivebusiness_bucket': 'hammertofall'}, auth=user.auth,
    ...
    def test_onedrivebusiness_set_bucket_registered(self):
        registration = self.project.register_node(
            get_default_metaschema(), Auth(self.user),
            DraftRegistrationFactory(branched_from=self.project), ''
        )

        url = registration.api_url_for('onedrivebusiness_set_config')
        res = self.app.put_json(
            url, {'onedrivebusiness_bucket': 'hammertofall'}, auth=self.user.auth,
            expect_errors=True,
        )

        assert_equal(res.status_code, http_status.HTTP_400_BAD_REQUEST)

    @mock.patch('addons.onedrivebusiness.views.utils.can_list', return_value=False)
    def test_user_settings_cant_list(self, mock_can_list):
        url = api_url_for('onedrivebusiness_add_user_account')
        rv = self.app.post_json(url, {
            'access_key': 'aldkjf',
            'secret_key': 'las'
        }, auth=self.user.auth, expect_errors=True)
    ...
    def test_onedrivebusiness_remove_node_settings_owner(self):
        url = self.node_settings.owner.api_url_for('onedrivebusiness_deauthorize_node')
    ...
    def test_onedrivebusiness_remove_node_settings_unauthorized(self):
        url = self.node_settings.owner.api_url_for('onedrivebusiness_deauthorize_node')
    ...
    @mock.patch('addons.onedrivebusiness.models.get_bucket_names')
    def test_folder_list(self, mock_names):
        mock_names.return_value = ['bucket1', 'bucket2']
        super(TestOneDriveBusinessViews, self).test_folder_list()

    @mock.patch('addons.onedrivebusiness.models.bucket_exists')
...
class TestCreateBucket(OneDriveBusinessAddonTestCase, OsfTestCase):
    ...
    def setUp(self):
        ...
        self.project.add_addon('onedrivebusiness', auth=self.consolidated_auth)
        self.project.creator.add_addon('onedrivebusiness')

        self.user_settings = self.user.get_addon('onedrivebusiness')
        self.user_settings.access_key = 'We-Will-Rock-You'
        self.user_settings.secret_key = 'Idontknowanyqueensongs'
        self.user_settings.save()

        self.node_settings = self.project.get_addon('onedrivebusiness')
        self.node_settings.bucket = 'Sheer-Heart-Attack'
        self.node_settings.user_settings = self.project.creator.get_addon('onedrivebusiness')
    ...
    @mock.patch('addons.onedrivebusiness.views.utils.create_bucket')
    @mock.patch('addons.onedrivebusiness.views.utils.get_bucket_names')
    def test_create_bucket_pass(self, mock_names, mock_make):
        ...
        url = self.project.api_url_for('onedrivebusiness_create_bucket')
    ...
    def test_create_bucket_fail(self, mock_make):
        ...
        url = '/api/v1/project/{0}/onedrivebusiness/newbucket/'.format(self.project._id)
        ret = self.app.post_json(url, {'bucket_name': 'doesntevenmatter'}, auth=self.user.auth, expect_errors=True)

        assert_equals(ret.body.decode(), '{"message": "This should work", "title": "Problem connecting to My MinIO"}')
