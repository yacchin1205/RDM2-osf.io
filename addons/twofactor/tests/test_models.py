...
class TestUserSettingsModel(unittest.TestCase):
    ...
    def test_otpauth_url(self):
        ...
        assert url.path == '/RDM:{}'.format(self.user.username)
        assert parse_qs(url.query) == {'secret': [self.TOTP_SECRET_B32]}

    def test_json(self):
        # url = 'otpauth://totp/RDM:{}?secret=' + self.TOTP_SECRET_B32

        settings = self.user_settings.to_json(user=None)
        assert settings == {'is_enabled': True, 'addon_full_name': 'Two-factor Authentication', 'addon_short_name': 'twofactor', 'drift': 0, 'is_confirmed': False, 'nodes': [], 'secret': self.TOTP_SECRET_B32, 'has_auth': False}
    ...
    def test_verify_valid_core_drift(self):
        ...
        assert self.user_settings.verify_code(_valid_code(self.TOTP_SECRET, drift=1))

        # make sure drift is updated.
        assert self.user_settings.totp_drift == 1

        # use a code from 60 seconds in the future
        assert self.user_settings.verify_code(_valid_code(self.TOTP_SECRET, drift=2))
