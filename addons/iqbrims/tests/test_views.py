...
class TestStatusViews(IQBRIMSAddonTestCase, OsfTestCase):
    ...
    def test_get_status_with_admin(self, mock_get_management_node):
        ...
        assert res.json['data']['id'] == self.project._id
        assert res.json['data']['type'] == 'iqbrims-status'
        assert list(res.json['data']['attributes'].keys()) == ['state', 'labo_list', 'review_folders', 'is_admin', 'task_url']
        assert res.json['data']['attributes']['state'] == 'initialized'
        assert len(res.json['data']['attributes']['labo_list']) == len(settings.LABO_LIST)
        assert res.json['data']['attributes']['review_folders'] == iqbrims_views.REVIEW_FOLDERS
    ...
    def test_set_status(self, mock_get_management_node):
        ...
        assert res.status_code == 200
        assert res.json == {'data': {'attributes': status, 'type': 'iqbrims-status', 'id': self.project._id}}
    ...
    def test_set_status_to_deposit(self, mock_get_management_node, mock_import_auth_from_management_node,
        ...
        assert mock_import_auth_from_management_node.call_args[0] == (self.project, iqbrims, fake_management_project)

        assert mock_iqbrims_init_folders.call_count == 1
        assert mock_iqbrims_init_folders.call_args[0] == (self.project, fake_management_project, status['state'], status['labo_id'])

        assert mock_update_spreadsheet.call_count == 1
        assert mock_update_spreadsheet.call_args[0] == (self.project, fake_management_project, status['state'], payload['data']['attributes'])

        assert mock_flowable_make_request.call_count == 1
        name, args, kwargs = mock_flowable_make_request.mock_calls[0]
        assert args == ('POST', settings.FLOWABLE_HOST + 'service/runtime/process-instances')
        assert json.loads(kwargs['data'])['processDefinitionId'] == settings.FLOWABLE_RESEARCH_APP_ID
        vars = json.loads(kwargs['data'])['variables']
        assert [v for v in vars if v['name'] == 'projectId'][0] == {'name': 'projectId', 'type': 'string', 'value': self.project._id}
        assert [v for v in vars if v['name'] == 'paperFolderPattern'][0] == {'name': 'paperFolderPattern', 'type': 'string', 'value': 'deposit/fake_labo_name/%-{}/'.format(self.project._id)}
    ...
    def test_set_status_to_check(self, mock_get_management_node, mock_import_auth_from_management_node,
        ...
        vars = json.loads(kwargs['data'])['variables']
        assert [v for v in vars if v['name'] == 'projectId'][0] == {'name': 'projectId', 'type': 'string', 'value': self.project._id}
        assert [v for v in vars if v['name'] == 'paperFolderPattern'][0] == {'name': 'paperFolderPattern', 'type': 'string', 'value': 'check/fake_labo_name/%-{}/'.format(self.project._id)}
    ...
    def test_set_status_to_custom_deposit(self, mock_get_management_node,
        ...
        assert mock_flowable_make_request.call_count == 1
        name, args, kwargs = mock_flowable_make_request.mock_calls[0]
        assert args == ('POST', 'https://test.somehost.ac.jp/' + 'service/runtime/process-instances')
...
class TestStorageViews(IQBRIMSAddonTestCase, OsfTestCase):
    ...
    def test_reject_checklist_storage(self, mock_files, mock_folders,
        ...
        foldername = cargs[1]
        assert re.match('(.*)\\.[0-9]+\\-[0-9]+', foldername).group(1) == u'チェックリスト'
    ...
    def test_reject_index_storage(self, mock_update_row,
        ...
        assert res.status_code == 200
        assert res.json == {'status': 'rejected', 'root_folder': 'iqb123/'}
    ...
    def test_create_index(self, mock_get_file_link,
        ...
        assert res.status_code == 200
        assert res.json == {'status': 'complete', 'url': 'https://a.b/sheet123'}
        ...
        assert mock_add_files.call_args == (('Files', 'ss123', 'Management', 'ss456', ['f1.txt', 'f2.txt', 'test/file3.txt', '']),)
    ...
    def test_create_index_custom_sheet(self, mock_get_file_link,
        ...
        assert mock_grant_access_from_anyone.call_args == (('sheet123',),)
        mock_add_files.assert_called_once()
        assert mock_add_files.call_args == (('Files', 'ss123', 'Files', 'ss123', ['f1.txt', 'f2.txt', 'test/file3.txt', '']),)
    ...
    def test_create_index_ja(self, mock_get_file_link,
        ...
        assert mock_grant_access_from_anyone.call_args == (('sheet123',),)
        mock_add_files.assert_called_once()
        assert mock_add_files.call_args == (('Files', 'ss123', 'Management', 'ss456', ['f1.txt', 'f2.txt', u'test/ファイル3.txt', '']),)
...
class TestWorkflowStateViews(IQBRIMSAddonTestCase, OsfTestCase):
    ...
    def test_post_minimal_workflow_state(self, mock_get_management_node):
        ...
        assert res.status_code == 200
        assert res.json == {'status': 'complete', 'data': {'state': 'initialized', 'workflow_raw_state': 'test', 'workflow_raw_permissions': ['READ', 'WRITE']}}
    ...
    def test_post_uploadable_workflow_state(self, mock_get_management_node,
        ...
        assert res.status_code == 200
        assert res.json == {'status': 'complete', 'data': {'labo_id': 'fake_labo_name', 'state': 'deposit', 'workflow_raw_state': 'test', 'workflow_raw_permissions': ['READ', 'WRITE', 'UPLOADABLE']}}
    ...
    def test_post_prevent_uploadable_workflow_state(self, mock_get_management_node,
        ...
        assert res.status_code == 200
        assert res.json == {'status': 'complete', 'data': {'labo_id': 'fake_labo_name', 'state': 'deposit', 'workflow_raw_state': 'test', 'workflow_raw_permissions': ['READ', 'WRITE']}}
    ...
    def test_post_custom_workflow_state(self, mock_get_management_node):
        ...
        assert res.status_code == 200
        assert res.json == {'status': 'complete', 'data': {'workflow_raw_state': 'test', 'workflow_raw_permissions': ['READ', 'WRITE'], 'state': 'initialized', 'is_directly_submit_data': True}}
