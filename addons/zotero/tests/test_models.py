...
class ZoteroNodeSettingsTestCase(OAuthCitationsNodeSettingsTestSuiteMixin, unittest.TestCase):
    ...
    def test_set_library(self):
        ...
        assert self.user_settings.verify_oauth_access(node=self.node, external_account=self.external_account, metadata={'library': 'fake-library-id'})

        log = self.node.logs.latest()
        assert log.action == '{}_library_selected'.format(self.short_name)
...
class ZoteroUserSettingsTestCase(OAuthAddonUserSettingTestSuiteMixin, unittest.TestCase):
    ...
    def test_verify_oauth_access_metadata_with_library(self):
        ...
        assert self.user_settings.verify_oauth_access(node=self.node, external_account=self.external_account, metadata={'library': 'fake_library_id'})

        assert not self.user_settings.verify_oauth_access(node=self.node, external_account=self.external_account, metadata={'library': 'another_library_id'})
