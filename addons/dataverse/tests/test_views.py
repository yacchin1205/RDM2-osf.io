...
class TestConfigViews(DataverseAddonTestCase, OAuthAddonConfigViewsTestCaseMixin, OsfTestCase):
    ...
    def test_set_config(self, mock_connection):
        ...
        self.project.reload()
        assert (self.project.logs.latest().action) == ('{0}_dataset_linked'.format(self.ADDON_SHORT_NAME))
        assert (res.json['dataverse']) == (self.connection.get_dataverse('ALIAS3').title)
        assert (res.json['dataset']) == (self.connection.get_dataverse('ALIAS3').get_dataset_by_doi('doi:12.3456/DVN/00003').title)
